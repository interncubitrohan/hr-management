...
import { BoxCubeIcon, CalenderIcon, ChevronDownIcon, GridIcon, HorizontaLDots, UserCircleIcon, } from "../icons";
...
const navItems = [
    ...
    },
    {
        ...
        name: "HR Management",
        subItems: [
            { name: "Applications", path: "/hr/applications" },
            { name: "Employees", path: "/hr/employees" },
            { name: "Documents", path: "/hr/documents" },
            { name: "Attendance", path: "/hr/attendance" },
            { name: "Leaves", path: "/hr/leaves" },
            { name: "Payroll", path: "/hr/payroll" },
            { name: "Reimbursements", path: "/hr/reimbursements" },
            { name: "Incidents", path: "/hr/incidents" },
...
const AppSidebar = () => {
    ...
    const renderMenuItems = (items, menuType) => (<ul className="flex flex-col gap-4">
      {items.map((nav, index) => (<li key={nav.name}>
          {nav.subItems ? (<button onClick={() => handleSubmenuToggle(index, menuType)} className={`menu-item group ${openSubmenu?.type === menuType && openSubmenu?.index === index
                    ? "menu-item-active"
              ...
              {(isExpanded || isHovered || isMobileOpen) && (<ChevronDownIcon className={`ml-auto w-5 h-5 transition-transform duration-200 ${openSubmenu?.type === menuType &&
