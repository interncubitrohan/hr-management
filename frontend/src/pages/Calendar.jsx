...
const Calendar = () => {
  ...
  useEffect(() => {
    ...
    setEvents([
      ...
      },
      {
        ...
        extendedProps: { calendar: "Academic" },
      },
      {
        id: "3",
  ...
  const handleFilterChange = (category) => {
    if (filterEvents.includes(category)) {
      setFilterEvents(filterEvents.filter((c) => c !== category));
    } else {
      setFilterEvents([...filterEvents, category]);
    }
  };

  const filteredEvents = events.filter((event) =>
    filterEvents.includes(event.extendedProps.calendar)
  );
  ...
  return (
    <>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex flex-wrap gap-4 items-center">
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Filters:</span>
          {Object.entries(calendarsEvents).map(([key, value]) => (
            <label key={key} className="flex items-center space-x-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer select-none">
              <input
                type="checkbox"
                checked={filterEvents.includes(key)}
                onChange={() => handleFilterChange(key)}
                className={`form-checkbox text-${value} rounded focus:ring-0`}
              />
              <span>{key}</span>
            </label>
          ))}
            ...
            <div className="mt-8">
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-wrap items-center gap-4 sm:gap-5">
                  {Object.entries(calendarsEvents).map(([key, value]) => (<div key={key} className="n-chk">
                    <div className={`form-check form-check-${value} form-check-inline`}>
                      <label className="flex items-center text-sm text-gray-700 form-check-label dark:text-gray-400" htmlFor={`modal${key}`}>
                        <span className="relative">
                          <input className="sr-only form-check-input" type="radio" name="event-level" value={key} id={`modal${key}`} checked={eventLevel === key} onChange={() => setEventLevel(key)} />
...
const renderEventContent = (eventInfo) => {
  ...
  return (<div className={`event-fc-color flex fc-event-main ${colorClass} p-1 rounded-sm`}>
